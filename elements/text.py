...
class TextEdit(PyGameUIHandler):
    def __init__(self, text: str, placeholder: str, font, rect: pygame.Rect, can_write = None):
        self.text = text
    ...
    def draw(self, surface, offset: Vector2i = Vector2i(0, 0)):
        ...
        for line in range(self.line_amount):
            if line in self.skip:
                x += 50
                continue
            pygame.draw.rect(surface,(0,0,0),(x + self.get_rect().x,self.get_rect().y + self.get_rect().height - 6,40,6))
